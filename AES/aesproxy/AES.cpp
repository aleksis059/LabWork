...
{	setlocale(LC_CTYPE,"Russian");
    ...
    while (cont)
    {
        std::cout << "Выберите режим: 1=OFB, 2=CFB, 3=CTR ,  4=CBC,  5=ECB\n";
        std::cin >> mode;
        if (mode > 5 || mode < 1)
        {
            system("cls");
            std::cout << "Ошибка, возможные номера режимов 1-5";
            _getch();
        }
        else cont = false;
        system("cls");
    ...
    std::cout << "Идет считывание plain.docx...";
    plain.ReadData();
    system("cls");
    std::cout << "Идет Шифрование plain.docx...";
    ...
    system("cls");
    std::cout << "Идет запись cypher.docx";
    cypher.GetData() = crypter.GetEncData();
    cypher.WriteData();
    system("cls");
    std::cout << "Идет дешифрование cypher.docx";
    ...
    system("cls");
    std::cout << "Запись дешифрования decrypted.docx";
